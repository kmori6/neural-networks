import math

import torch
import torch.nn as nn


class ConvolutionSubsampling(nn.Module):
    def __init__(self, output_size: int):
        super().__init__()
        self.conv1 = nn.Conv2d(1, output_size, kernel_size=3, stride=2)
        self.conv2 = nn.Conv2d(output_size, output_size, kernel_size=3, stride=2)
        self.activation = nn.ReLU()

    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).
            mask (torch.Tensor): Mask (batch, 1, frame)

        Returns:
            tuple[torch.Tensor, torch.Tensor]: _description_
        """
        x = x[:, None, :, :]  # (batch, 1, frame, feat_size)
        x = self.conv1(x)  # (batch, output_size, frame, feat_size)
        x = self.activation(x)
        x = self.conv2(x)
        x = self.activation(x)
        x = x.transpose(1, 2).flatten(2, -1)  # (batch, frame, output_size * feat_size')
        return x, mask[:, :, :-2:2][:, :, :-2:2]


class RelativePositionalEncoding(nn.Module):
    def __init__(self, d_model: int, max_length: int = 4096):
        super().__init__()
        self.d_model = d_model
        self.init_pos_embed(max_length)

    def init_pos_embed(self, max_length: int):
        t = torch.arange(max_length - 1, -max_length, -1, dtype=torch.float32)
        omega = torch.exp(-torch.arange(0, self.d_model, 2, dtype=torch.float32) / self.d_model * math.log(10000.0))
        theta = omega[None, :] * t[:, None]  # (2 * max_length - 1, d_model / 2)
        pos_embed = torch.zeros(2 * max_length - 1, self.d_model)
        pos_embed[:, 0::2] = torch.sin(theta)
        pos_embed[:, 1::2] = torch.cos(theta)
        self.register_buffer("pos_embed", pos_embed[None, :, :], persistent=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).

        Returns:
            torch.Tensor: Position embedding (batch, 2 * frame - 1, d_model).
        """
        base_position = self.pos_embed.shape[1] // 2
        pos_embed = self.pos_embed[:, base_position - x.shape[1] + 1 : base_position + x.shape[1]]
        return pos_embed


class FeedForwardModule(nn.Module):
    def __init__(self, d_model: int, dropout_rate: float):
        super().__init__()
        self.layernorm = nn.LayerNorm(d_model)
        self.linear_layer1 = nn.Linear(d_model, 4 * d_model)
        self.swish_activation = nn.SiLU()
        self.dropout1 = nn.Dropout(dropout_rate)
        self.linear_layer2 = nn.Linear(4 * d_model, d_model)
        self.dropout2 = nn.Dropout(dropout_rate)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).

        Returns:
            torch.Tensor: Output sequence (batch, frame, d_model).
        """
        x = self.layernorm(x)
        x = self.linear_layer1(x)
        x = self.swish_activation(x)
        x = self.dropout1(x)
        x = self.linear_layer2(x)
        x = self.dropout2(x)
        return x


class MultiHeadSelfAttentionModule(nn.Module):
    def __init__(self, d_model: int, num_heads: int, dropout_rate: float):
        super().__init__()
        assert d_model % num_heads == 0
        self.h = num_heads
        self.d_model = d_model
        self.d_k = d_model // num_heads
        self.layernorm = nn.LayerNorm(d_model)
        # multi-head attention with relative positional embedding
        self.w_q = nn.Linear(d_model, d_model)
        self.w_k = nn.Linear(d_model, d_model)
        self.w_v = nn.Linear(d_model, d_model)
        self.w_p = nn.Linear(d_model, d_model, bias=False)
        self.w_o = nn.Linear(d_model, d_model)
        self.b_u = nn.Parameter(torch.Tensor(self.h, self.d_k))
        self.b_v = nn.Parameter(torch.Tensor(self.h, self.d_k))
        self.dropout1 = nn.Dropout(dropout_rate)
        nn.init.xavier_uniform_(self.b_u)
        nn.init.xavier_uniform_(self.b_v)
        self.dropout2 = nn.Dropout(dropout_rate)

    def forward(self, x: torch.Tensor, p: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).
            p (torch.Tensor): Position embedding (batch, 2 * frame - 1, d_model).
            mask (torch.Tensor): Mask (batch, 1, 1, frame).

        Returns:
            torch.Tensor: Output sequence (batch, frame, d_model).
        """
        x = self.layernorm(x)
        # multi-head attention with relative positional embedding
        # described in https://arxiv.org/abs/1901.02860
        b, t, _ = x.shape
        q = self.w_q(x).view(b, t, self.h, self.d_k).transpose(1, 2)  # (batch, head, frame, d_k)
        k = self.w_k(x).view(b, t, self.h, self.d_k).transpose(1, 2)  # (batch, head, frame, d_k)
        v = self.w_v(x).view(b, t, self.h, self.d_k).transpose(1, 2)  # (batch, head, frame, d_k)
        p = self.w_p(p).view(1, -1, self.h, self.d_k).transpose(1, 2)  # (batch, head, 2 * frame - 1, d_k)
        # attention score in section 3.3
        ac = torch.matmul(q + self.b_u[None, :, None, :], k.transpose(2, 3))  # (batch, head, frame, frame)
        # efficient computation in appendix B
        bd = torch.matmul(q + self.b_v[None, :, None, :], p.transpose(2, 3))  # (batch, head, frame, 2 * frame - 1)
        bd = bd.flatten(2, 3)[..., t - 1 :].unfold(-1, t, bd.shape[-1] - 1)  # (batch, head, frame, frame)
        x = (ac + bd) / math.sqrt(self.d_k)  # (batch, head, frame, frame)
        x = x.masked_fill(~mask, float("-inf"))
        x = torch.softmax(x, dim=-1)
        x = self.dropout1(x)
        x = torch.matmul(x, v)  # (batch, head, frame, d_k)
        x = x.transpose(1, 2).flatten(2, 3)  # (batch, frame, d_model)
        x = self.w_o(x)

        x = self.dropout2(x)
        return x


class ConvolutionModule(nn.Module):
    def __init__(self, d_model: int, kernel_size: int, dropout_rate: float):
        super().__init__()
        self.layernorm = nn.LayerNorm(d_model)
        self.pointwise_conv1 = nn.Conv1d(d_model, 2 * d_model, kernel_size=1, stride=1, padding=0)
        self.glu_activation = nn.GLU(dim=1)
        self.depthwise_conv = nn.Conv1d(
            d_model, d_model, kernel_size=kernel_size, stride=1, padding=kernel_size // 2, groups=d_model
        )
        self.batchnorm = nn.BatchNorm1d(d_model)
        self.swish_activation = nn.SiLU()
        self.pointwise_conv2 = nn.Conv1d(d_model, d_model, kernel_size=1, stride=1, padding=0)
        self.dropout = nn.Dropout(dropout_rate)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).

        Returns:
            torch.Tensor: Output sequence (batch, frame, d_model).
        """
        x = self.layernorm(x)
        x = x.transpose(1, 2)  # (batch, d_model, frame)
        x = self.pointwise_conv1(x)  # (batch, 2 * d_model, frame)
        x = self.glu_activation(x)  # (batch, d_model, frame)
        x = self.depthwise_conv(x)
        x = self.batchnorm(x)
        x = self.swish_activation(x)
        x = self.pointwise_conv2(x)
        x = x.transpose(1, 2)  # (batch, frame, d_model)
        x = self.dropout(x)
        return x


class ConformerBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, kernel_size: int, dropout_rate: float):
        super().__init__()
        self.ffn1 = FeedForwardModule(d_model, dropout_rate)
        self.mhsa = MultiHeadSelfAttentionModule(d_model, num_heads, dropout_rate)
        self.conv = ConvolutionModule(d_model, kernel_size, dropout_rate)
        self.ffn2 = FeedForwardModule(d_model, dropout_rate)
        self.layernorm = nn.LayerNorm(d_model)

    def forward(self, x: torch.Tensor, p: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, d_model).
            p (torch.Tensor): Position embedding (batch, 2 * frame - 1, d_model).
            mask (torch.Tensor): Mask (batch, 1, 1, frame).

        Returns:
            torch.Tensor: Output sequence (batch, frame, d_model).
        """
        x = x + 0.5 * self.ffn1(x)
        x = x + self.mhsa(x, p, mask)
        x = x + self.conv(x)
        x = self.layernorm(x + 0.5 * self.ffn2(x))
        return x


class Conformer(nn.Module):
    def __init__(
        self,
        input_size: int = 80,
        d_model: int = 512,
        num_heads: int = 8,
        kernel_size: int = 31,
        num_blocks: int = 17,
        dropout_rate: float = 0.1,
    ):
        super().__init__()
        self.d_model = d_model
        self.scale = math.sqrt(d_model)
        self.convolution_subsampling = ConvolutionSubsampling(d_model)
        self.linear = nn.Linear(d_model * (((input_size - 1) // 2 - 1) // 2), d_model)
        self.pos_encoding = RelativePositionalEncoding(d_model)
        self.dropout1 = nn.Dropout(dropout_rate)
        self.dropout2 = nn.Dropout(dropout_rate)
        self.conformer_blocks = nn.ModuleList(
            [ConformerBlock(d_model, num_heads, kernel_size, dropout_rate) for _ in range(num_blocks)]
        )

    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """

        Args:
            x (torch.Tensor): Input sequence (batch, frame, input_size).
            mask (torch.Tensor): Mask (batch, 1, frame).

        Returns:
            torch.Tensor: Output sequence (batch, frame', d_model).
        """
        x, mask = self.convolution_subsampling(x, mask)
        x = self.linear(x) * self.scale
        x = self.dropout1(x)
        p = self.pos_encoding(x)
        p = self.dropout2(p)
        mask = mask[:, None, :, :]
        for block in self.conformer_blocks:
            x = block(x, p, mask)
        return x, mask
